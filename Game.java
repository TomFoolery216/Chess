...
//	Squares squares = new Squares();
	...
	public void initPlayer() // This makes an array of players 
	{
		for (int i = 0; i < 2; i++)
		{
			player [i] = new Player();
	...
	{ 
	for (int i =0; i < 1; i++){ // This is an array of pieces 32 are created. Each piece catagory is set to a name, color, player, row and column. 
		pieces[i] = new King();
	...
	public Piece getPiece(Squares x) // This function returns the piece when the square it's on is called
    {
         for (int i =0; i < 32; i++)
	{
		
		if (x.getRow()==pieces[i].getRow() && x.getColumn()==pieces[i].getColumn())
	...
	public Squares getSqs(String s)
    {
     ...
     public void selectSquare(Piece piece, Squares squares, Squares squares2) throws InvalidMoveException// This passes error messages to the tester if the user makes a mistake
	{
	     try
		{
		if(getPiece(squares) == null)
		{
			InvalidMoveException exeception = new InvalidMoveException("\nPiece is not on the Square yo!");
			throw exeception;
	...
	public boolean pickUpPiece(Piece piece, Squares squares, Squares squares2) throws InvalidMoveException// This passes error messages to the tester if the user makes a mistake
	{
	     try
		{
		if(getPiece(squares) == null)
